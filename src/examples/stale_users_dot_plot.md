...
A dot plot showing the number of days since last login per user. User defined here as the username concatenated with the server hostname.
- **Report category:** Database activity

- **Required report headers:**
    - **Primary category:** Server hostname
    - **Secondary category:** DB user
    - **Tertiary category:** Max (Connection start (local time))
    - **Primary value:** Count
```js
...
<div class="tip">Filter the base report for this chart to include only successful logins to avoid failed logins tainting the results.</div>
...
```js 
    ...
    "transform": [
        {
            "calculate": "datum['Server hostname'] + '::' + datum['DB user']",
            "as": "serveruser"
        },
        {
    ...
    "encoding": {
        ...
        "x": {
            "field": "Max (Connection start (local time))",
            "type": "temporal",
            "axis": {
                "grid": false
            },
